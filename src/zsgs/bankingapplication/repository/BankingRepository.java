...
import zsgs.bankingapplication.dao.User;
...
public class BankingRepository {
	...
	private ArrayList<BankTransaction> accountCredentials = new ArrayList<>();
	...
	public long checkDeposit(int acNo,long balance,Date date)
	{
		...
		for (User uc : UserCredentials) {
			if(uc.getAccountNo()==acNo)
			{
				...
				accountCredentials.add(new BankTransaction(acNo,balance,date,"Deposit",updatedBalance));		
			}
	...
	public long checkWithdraw(int acNo,long balance,Date date)
	{
		...
		for (User uc : UserCredentials) {
			if(uc.getAccountNo()==acNo)
			{
				...
				updatedBalance=uc.getAccountBalance()-balance;
				uc.setAccountBalance(updatedBalance);
				accountCredentials.add(new BankTransaction(acNo,balance,date,"Withdraw",updatedBalance));
				}
			}
	...
	public ArrayList<BankTransaction> viewTransection(int AccountNo)
	{
		ArrayList <BankTransaction> viewTransection =new ArrayList<>();
		for(BankTransaction tc: accountCredentials)
		{
