...
import zsgs.bankingapplication.login.*;
...
public class BankingView {
	...
	private void transactionHistory() {
		...
		bankingcontroller.checkTrasactionHistory(accountNo);
		;
	}
	...
	private void viewAccountBalance() {
		System.out.println("Enter AccountNo");
		int accountNo = sc.nextInt();
		bankingcontroller.checkBalance(accountNo);
	...
	private void depositFunds() {
		...
		try {
			...
			bankingcontroller.checkDeposit(accountNo, amount, date);
	...
	private void withdrawFunds() {
		...
		try {
			Date date = dateFormat.parse(dateInput);
			bankingcontroller.checkWithDrawFund(accountNo, amount, date);
		} catch (ParseException e) {
			e.printStackTrace();
		}
	}
	...
	public void transectionHistory(ArrayList<BankTransaction> userlist) {
		System.out.println("--------------------------------------------------");
		System.out.println("Transection History");
		for (BankTransaction us : userlist)
			System.out.println(us.getAccountNumber() + " " + us.getBalance() + " " + us.getTransectionType() + " "
					+ us.getDate() + " " + us.getAccountbalance());
